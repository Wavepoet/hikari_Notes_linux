...
graph TB
    %% 定义深色模式全局样式
    classDef default fill:#1a1a1a,stroke:#ccc,color:#fff;
    classDef master fill:#111,stroke:#fff,stroke-width:2px,stroke-dasharray: 5 5,color:#fff;
    classDef node fill:#1a1a1a,stroke:#ccc,color:#fff;
    classDef pod fill:#111,stroke:#999,color:#fff;
    classDef storage fill:#000,stroke:#fff,color:#fff,stroke-width:2px;

    subgraph User_Space [用户层]
        kubectl[kubectl 用户命令]
    end

    subgraph Master_Node [Master 控制平面]
        direction TB
        subgraph APIs [API Server]
            auth[authentication<br/>authorization]
            rest["REST (pods, services,<br/>rep. controllers)"]
            actuator[scheduling<br/>actuator]
        ...
        scheduler[Scheduler]
        ctrl_mgr["controller manager<br/>(replication controller etc.)"]
        etcd[("Distributed<br/>Watchable<br/>Storage<br/>(implemented via etcd)")]
    ...
    subgraph Node_1 [Worker Node 1]
        ...
        subgraph Docker_1 [Docker Runtime]
            subgraph Pod_1a [Pod]
                cadvisor_1[cAdvisor]
            end
            subgraph Pod_1b [Pod]
    ...
    subgraph Node_2 [Worker Node 2]
        ...
        subgraph Docker_2 [Docker Runtime]
            subgraph Pod_2a [Pod]
                cadvisor_2[cAdvisor]
            end
            subgraph Pod_2b [Pod]
    ...
    %% 应用样式
    class Master_Node master;
    class Node_1,Node_2 node;
    class Pod_1a,Pod_1b,Pod_1c,Pod_2a,Pod_2b pod;
    class etcd storage;
    
    %% 外部节点文字颜色
    style kubectl color:#fff;
    style Internet color:#fff;
    style Firewall color:#fff;
```
...
graph TB
    %% 深色模式样式定义
    classDef default fill:#1a1a1a,stroke:#ccc,color:#fff;
    classDef outline fill:transparent,stroke:#fff,stroke-width:1.5px,stroke-dasharray: 5 5,color:#fff;
    classDef box fill:#2d2d2d,stroke:#bbb,stroke-width:1px,color:#fff;

    Kubectl["Kubectl"]

    subgraph Control_Plane ["控制层面 (Control Plane)"]
        direction TB
        etcd["etcd"]
        API["Kube-APIServer"]
        Scheduler["kube-Scheduler"]
        CM["kube-Controller Manager"]
        CCM["kube-Cloud Controller Manager"]
    ...
    subgraph Worker_Nodes ["工作节点 (Worker Nodes)"]
        direction TB
        Kubelet["Kubelet"]
    ...
    %% 应用样式
    class Control_Plane,Worker_Nodes outline;
    class Kubectl,API,etcd,Scheduler,CM,CCM,Kubelet,Runtime,Proxy,Pod,Svc box;
```
...
![images2.jpg](images/images2.jpg.jpg)
